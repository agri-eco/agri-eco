...
if ($products->num_rows > 0) {
    ...
    while ($ir = $inventory->fetch_assoc()) {
        $inv[] = $ir;
        $sold = $conn->query("SELECT sum(quantity) as total FROM order_list where product_id='{$ir['id']}' ")->fetch_assoc()['total'];
...
<section class="py-5 bg-light">
    <div class="container px-4 px-lg-5 mt-5">
        ...
        <div class="row gx-4 gx-lg-5 row-cols-1 row-cols-md-3 row-cols-xl-4 justify-content-center">
            <?php
            $products = $conn->query("SELECT p.*,i.quantity FROM products p inner join inventory i on p.id = i.id where p.status = 1 and (p.category_id = '{$category_id}' or p.sub_category_id = '{$sub_category_id}') and p.id !='{$id}' and i.quantity > 0 order by rand() limit 4 ");
