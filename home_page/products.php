            ...
            <div class="container-fluid p-0">
                ...
                <div class="tab-content pt-2">
                    <div class="tab-pane fade show active" id="book">
                        ...
                        <div class="row gx-2 gx-lg-2 row-cols-3 row-cols-md-3 row-cols-xl-3">

                            <?php
                            $whereData = "";
                            if (isset($_GET['search']))
                                $whereData = " and (product_name LIKE '%{$_GET['search']}%')";
                            elseif (isset($_GET['c']) && isset($_GET['s']))
                                $whereData = " and (md5(category_id) = '{$_GET['c']}' )";
                            elseif (isset($_GET['c']) && !isset($_GET['s']))
                                $whereData = " and md5(category_id) = '{$_GET['c']}' ";

                            $products = $conn->query("SELECT p.*,i.quantity FROM products p inner join inventory i on p.id=i.product_id where i.quantity > 0 and p.status = 1 {$whereData} order by rand() ");
                            ...
                            ?>
                                <div class="col-md-12 mb-5">
                                    <div class="card product-item h-100">
                                        <!-- Product image-->
                                        <img class="card-img-top p-1 w-100" src="<?php echo validate_image($img) ?>" height="300rem" style="object-fit:cover" loading="lazy" alt="..." />
                                        <!-- Product details-->
                                        <div class="card-body p-4">
                                            ...
                                            <p class="m-0"><small>By: <?php echo $row['product_name'] ?></small></p>
                                        </div>
                                        <!-- Product actions-->
                    ...
                    <div class="tab-pane fade" id="details">
                        <h3 class="text-center"><?php echo $product_name . " Category" ?></h3>
                        <hr>
                        <div>
