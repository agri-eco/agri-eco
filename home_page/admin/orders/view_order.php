...
<div class="card card-outline card-primary">
    <div class="card-body">
        <div class="conitaner-fluid">
            ...
            <?php if ($order_type == 1) : ?>
                <p><b>Delivery Address: <?php echo $delivery_address, " ", $barangay, " ", $city, ", ", $province ?></b></p>
            <?php elseif ($order_type == 2) : ?>
                <p><b>Pick up Address: <?php echo 'Cavite State University Main Campus - Bancod I, Indang, Cavite' ?></b></p>
            <?php endif; ?>
            <table class="table-striped table table-bordered">
                ...
                <tbody>
                    <?php
                    $olist = $conn->query("SELECT o.*,p.product_name,p.department FROM order_list o inner join products p on o.product_id = p.id where o.order_id = '{$id}' ");
                    ...
                    ?>
                        <tr>
                            ...
                            <td>
                                ...
                                <p class="m-0"><small>Manufacturer: <?php echo $row['department'] ?></small></p>

                            </td>
                            <td class="text-right">₱ <?php echo (isset($row['price']) ? number_format($row['price'], 2) : 0.00 * $row['quantity']) ?></td>
                            <td class="text-right">₱ <?php echo isset($row['price']) * isset($row['quantity']) ? number_format($row['price'] * $row['quantity'], 2) : 0.00 ?></td>
                        </tr>
                    <?php endwhile; ?>
                </tbody>
                <tfoot>
                    <tr>
                        ...
                        <th class="text-right"><?php echo "₱ ", isset($amount) ? number_format($amount, 2) : 0.00 ?></th>
                    </tr>
                </tfoot>
            </table>
        </div>
        <div class="row">
            <div class="col-6">
                ...
                <p>Order Type: <?php echo $order_type == 1 ? '<span class="badge badge-light text-dark">For Delivery</span>' : '<span class="badge badge-light text-dark">Pick up in Store</span>' ?></p>
            </div>
...
<script>
    $(function() {
        $('#update_status').click(function() {
            uni_modal("Update Status", "./orders/update_status.php?oid=<?php echo $id ?>&status=<?php echo $status ?>&otype=<?php echo $order_type ?>")
        })
    })
</script>