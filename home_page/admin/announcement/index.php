...
<div class="card card-outline card-primary">
	<div class="card-header">
		...
		<?php
		$count = $conn->query("SELECT count(id) as total from `headlines`  ")->fetch_assoc()['total'];
		if ($count < 5) {
		?>
				...
				<tbody>
					...
					?>
						<tr>
							...
							<td>
								<p class="truncate-1 m-0"><?php echo $row['announcements'] ?></p>
							</td>
							<td align="center">
...
<script>
	$(document).ready(function() {
		...
		$('.many_data').click(function() {
			alert_toast("Too many Announcement. Delete some.", 'error');
		})
		$('.table').dataTable();
	})
