            ...
            <table class="table table-bordered">
                ...
                <tbody>
                    ...
                    while ($row = $qry->fetch_assoc()) :
                        $olist = $conn->query("SELECT ol.*,p.product_name,p.department,concat(c.firstname,' ',c.lastname) as name,c.email,o.date_created FROM order_list ol inner join orders o on o.id = ol.order_id inner join `products` p on p.id = ol.product_id inner join clients c on c.id = o.client_id  where ol.order_id = '{$row['order_id']}' && o.paid = '1' ");
                    ...
                    ?>
                            <tr>
                                ...
                                <td>
                                    <p class="m-0"><?php echo $roww['product_name'] ?></p>
                                    <p class="m-0"><small>By: <?php echo $roww['department'] ?></small></p>
                                </td>
                                <td>
                                ...
                                <td class="text-right">₱ <?php echo isset($roww['price']) * isset($roww['quantity']) ? number_format($roww['price'] * $roww['quantity'], 2) : 0.00 ?></td>
                            </tr>
                        <?php endwhile; ?>
                    <?php endwhile; ?>
